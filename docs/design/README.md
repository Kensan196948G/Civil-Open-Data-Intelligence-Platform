...
| 項目 | 設計 | 実装 | 理由 |
| --- | --- | --- | --- |
| 地図タイル | OpenStreetMap | 地理院タイル (現行実装を維持) | 設計の map view は「プレースホルダー」と明記。実機能 (GSI タイル + 実標高API) を優先 |
| 標高取得 | 疑似乱数 | `/api/map/elevation` (実API) | 同上 |
| 監査ログのデータ | クライアント内デモデータ | `fetch_logs` 由来の実イベント (接続確認・サンプル取得)。操作系証跡は AuditLog モデル導入後に拡張 (Issue 起票) | DB スキーマ変更を伴うため段階導入 |
