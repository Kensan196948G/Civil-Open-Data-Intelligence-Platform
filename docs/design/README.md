...
| terrain | `/terrain` | ⛰️ 地形分析 | 標高・傾斜・縦断からの現場条件評価 |
| weather | `/weather` | 🌦️ 気象・海象 | 観測値・予報と閾値による作業条件の確認 |
| sites | `/sites` | 🚧 現場管理 | 現場マスタと観測地点の対応づけ |
| decisions | `/decisions` | 🧭 施工可否判定 | 閾値と観測値にもとづく判断と根拠の記録 |
| reports | `/reports` | 📊 レポート | 期間・現場を指定した集計と出力 |
| watchlist | `/watchlist` | 🔔 ウォッチリスト | 現場・データソースの変化を追跡 |

> 注: 上表は `src/components/AppHeader.tsx` の `VIEW_META` と一致している必要がある。
> 追加・欠落があると別画面のタイトルが誤表示されるため、`tests/unit/app-header-routes.test.ts`
> が一覧の完全一致を機械的に検証している（PR #183 導入）。

## シェル構成

- 左サイドバー 250px 固定 (`--side` 白)。ロゴブロック (アクセント色タイル + "Civil Open Data / Intelligence Platform")
- ナビ 4 グループ: 「運用」(ダッシュボード) / 「データ台帳」(データソース・地図) / 「地形・気象判断」(地形分析・気象海象・現場管理・施工可否判定・レポート) / 「記録・管理」(取得ログ・監査ログ・ウォッチリスト・タグ管理・設定)
