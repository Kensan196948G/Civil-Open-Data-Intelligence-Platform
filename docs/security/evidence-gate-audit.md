...
> 🔁 更新: 2026-08-12｜ 更新者: backend｜ 範囲: **§3.6 の新設のみ**（欠陥クラス「緑 ≠ 網羅」）。分類・件数・採番・§0〜§2 の判定、および §3.5 までの記述は変更していない。本節は evidence ゲート実装から独立した**読み手側**の欠陥記録であり、未マージの PR が含む実装変更は主張していない
...
| 2 | `restoreDrillStatus is success`（`check-neon-backup-evidence.js:241-245`） | `neon-backup.yml:69` の `inputs.restore_drill_status` または `vars.CODIP_LAST_RESTORE_DRILL_STATUS` → `:195` `--restore-drill-status` → `create-neon-backup-evidence.js:297`。既定値は撤廃され、未指定なら `:277-279` で異常終了する | 🔴 | 訓練を実施せずに `success` と申告すればゲートは通る。**ただし「何もしない」では通らない**: 値が無ければ `neon-backup.yml:114` の `Validate backup inputs` でジョブが落ちる | 分類は 🔴 のまま（**是正漏れではない**）。訓練は人間の手順であり実測へ変換できない。到達点は申告の裏付けであり、`--restore-drill-record` で [復旧訓練 実施記録](../runbooks/restore-drill-record.md) の台帳行を参照させ、`check-*.js:256-260` が非ゲートで可視化する |
...
| 4 | `lastRestoreDrillAt is fresh`（30日以内。`check-neon-backup-evidence.js:223-227`、閾値は `:7`） | `neon-backup.yml:68` の `inputs.restore_drill_at` または `vars.CODIP_LAST_RESTORE_DRILL_AT` → `:194` `--restore-drill-at` | 🔴 | 復旧訓練を実施していなくても、dispatch 時に本日の日付を入力するか `vars.CODIP_LAST_RESTORE_DRILL_AT` を更新すれば「30日以内」を満たす | 訓練実施の証跡（訓練 workflow の run ID・成果物）を供給元とし、自己申告の日付は上書き扱いで記録だけ残す。現状の緩和は #2 と同じく台帳参照のみ |
...
| 6 | `owner` 必須（`check-neon-backup-evidence.js:10-20` の必須フィールド） | `neon-backup.yml:71` の `inputs.owner` または `vars.CODIP_BACKUP_OWNER` → `:184` 既定値 `release-manager` | 🔴 | 実在しない担当者名でも通る。ただし責任者の記録が目的であり、機械検証の対象として設計されていない | 分類は 🔴 だが是正不要。**証跡の記録**であって**状態の検査**ではないことを文書側で明示する |
...
| # | ゲート名（検査項目） | 供給元（ファイル:行） | 分類 | 偽陰性シナリオ | 是正案 |
| ---: | --- | --- | :---: | --- | --- |
| 7 | Monitoring evidence 7項目（`Cloudflare Access evidence recorded` ほか。`production-evidence-report.js:424-430`、判定関数は `:283-304`） | `ci.yml:194-200` の `vars.CODIP_CLOUDFLARE_ACCESS_EVIDENCE` / `CODIP_MONITORING_CONTACTS` / `CODIP_CLOUDFLARE_ALERT_POLICY` / `CODIP_CLOUDFLARE_LOGS_EVIDENCE` / `CODIP_NEON_MONITORING_EVIDENCE` / `CODIP_SMOKE_MONITORING_SCHEDULE` / `CODIP_ROLLBACK_OWNER` → `production-evidence-report.js:44-52` | 🔴 | 🗓️ **2026-08-11（QA 調査時点）の所見**: `evidenceState()` は「空でない」かつ「placeholder 正規表現に一致しない」だけを見る。**GitHub Variables に `ok` の2文字を入れれば7項目すべてが ✅ になる。** アラートポリシーが存在しなくても、監視連絡先が失効していても通る。→ この経路は **Issue #128 で閉じた**（現況は §2.5 S7 が実行検査する） | **是正済み（Issue #128）。** `EVIDENCE_FORMATS`（`:137-219`）で変数ごとに形を固定し、`evidenceFormatState()`（`:283-304`）だけが ✅ を出せる（書式仕様の無い鍵は fail-close で拒否する。`:293`）。ISO 8601 日付・cron 式・連絡先ハンドル・空白なし識別子など、**人間が場当たりに書けない形**を要求する。分類は 🔴 のまま（供給元は `vars.*` のままで、形式が正しい嘘は依然書ける）。API での存在確認は #9 / #10 と同じく Cloudflare 読取権限が要るため未着手 |
| 8 | `Backup/restore evidence recorded`（`production-evidence-report.js:431`） | `ci.yml:201` `vars.CODIP_BACKUP_RESTORE_EVIDENCE` → `production-evidence-report.js:54-56` | 🔴 | 🗓️ **2026-08-11（QA 調査時点）の所見**: #7 と同じ。§2.1 の Neon 側ゲートとは独立しており、こちらは文字列の非空判定のみ。→ **Issue #128 で閉じた**（現況は §2.5 S8） | **是正済み（Issue #128）。** PITR window + 訓練日（ISO 8601）+ 訓練結果（`DRILL_OUTCOMES` のいずれか）を要求する（`:196-218`）。残る課題は供給元の一本化で、`release:check-neon-backup-evidence` の判定結果を供給元にする案は未着手（証跡変数の二重管理が残っている） |
| 9 | `CODIP_NEON_BRANCH` / `CODIP_HYPERDRIVE_BINDING` 必須・非 placeholder（`validate-production-target-env.js:106-110`） | `ci.yml:186-187` の `vars.*` | 🔴 | 実際のデプロイ先と異なる branch 名・binding 名でも、placeholder 語（`:6-14`）を含まなければ通る。誤った Neon branch を指したまま production 判定が成立する | `wrangler.jsonc` の `hyperdrive[].binding` および Neon API の branch 一覧と突合する |
| 10 | `CODIP_DISABLE_TOKEN_AUTH=true` / `CODIP_TRUST_PROXY_AUTH=true`（`validate-production-target-env.js:121-127`） | `ci.yml:189-190` の `vars.*` | 🔴 | **認証方式の宣言と実際のデプロイ設定が乖離しても検知しない。** Variables が `true` でも、デプロイ済み Worker の実効設定が異なれば直接トークン認証が生きたまま通る | デプロイ後の実挙動で確認する（未認証リクエストが Access へリダイレクトされることを smoke で確認済み。その結果を判定に接続する） |
| 11 | `DATABASE_URL` / `CODIP_MIGRATION_DATABASE_URL` の形式（postgres・sslmode・非 localhost。`validate-production-target-env.js:42-67, 90-91`） | `ci.yml:181-182` の `secrets.*` | 🔴 | 形式のみの検査で**接続は行わない**。到達不能・権限不足の URL でもこのゲートは通る | 緩和要因あり: 同 job の `db:pg:check-drift`（#14）が同じ `DATABASE_URL` で実接続するため、後段で失敗する。ゲート名を「形式契約」と明示すれば足りる |
| 12 | `CODIP_BASE_URL` が production ホストと一致（`validate-production-target-env.js:93-104`、定数は `:15`） | `ci.yml:185` の `vars.CODIP_BASE_URL` だが、`:99-101` で `odip.mirai-dx-platform.com` に**ピン留め**されている | 🟢 | — | **良い型の実例。** 自己申告の値でも、期待値をコード側の定数に固定すれば実質的に改竄余地が消える。#9 もこの形に寄せられる |
...
| 15 | PostGIS DDL 検査（`db:pg:check-postgis-ddl`、`ci.yml:229`） | `check-postgis-standard-record-ddl.js`（自前の I/O を持たず、呼び出し側の Prisma 接続経由） | 🟢 | — | 現状維持 |
| 16 | production smoke（`ci.yml:235,239`） | `release-smoke.js:27` `fetch(url, …)`、対象は `:119` `--base-url`（#12 でピン留め済み） | 🟢 | ⚠️ **供給元は実測でも判定が弱い例**（§1.1 の一種）。QA 調査時点の CSP 判定は部分文字列照合で、`connect-src` を丸ごと削除しても `img-src` 側の同一オリジン出現だけで条件が成立し、許可元の追加も検知しなかった。→ **Issue #129 で閉じた**（現況は §2.5 S16） | **是正済み（Issue #129）。** `scripts/tools/csp-contract.js` の `PINNED_DIRECTIVES` と突合し、ディレクティブの増減・許可元の増減・report-only の追加・ヘッダ重複を検知する。期待値は E2E と共有の単一定義で、本番構成へピン留めする。**供給元が 🟢 実測でも判定は別途検算が要る**ことの実例 |
| 17 | Cloudflare build artifact（`ci.yml:223` → `:226`） | `check-cloudflare-build-artifact.js:18-26` が `.open-next/` の実ファイルを `existsSync`/`statSync` | 🟡 | 同一 job の `npm run cf:build` が生成した成果物を同一 job が検査する。**本番へデプロイされた成果物とは独立**。ビルドは通るがデプロイが古いままでも検知しない | デプロイ後に `wrangler deployments list` の結果と突合する（現状は release notes への手貼り運用。レポート側は `inspectWrangler()` が `wrangler.jsonc` の**設定**を読むだけで、実デプロイとは突合していない） |
...
| 20 | production env 契約（synthetic） | `release-gate.js:63-70` がスクリプト内リテラルの `DATABASE_URL`（`example.com`）・`CODIP_ADMIN_TOKEN` を渡す。CI 版は `ci.yml:130-135` | 🔴 | **本番の env が壊れていてもこのゲートは通る。** 検査しているのは validator の挙動であって production の状態ではない | 是正不要だが名称の明確化を推奨。`release-gate.js:63` は既に `(synthetic)` と自己申告しており誠実。CI 側も同様に自己申告済み: ステップ名 `ci.yml:129` が `(synthetic values)` を含み、`:122-128` のコメントが「緑でも production の証跡ではない」ことを明示する（注記は 7f72626 で追加済みであり、当初指摘した欠落は解消している） |
| 21 | SQLite 前提の DB ゲート群（`db:migrate` / `db:check-duplicates` / `db:check-standard-record-policy` / `db:prune --dry-run`。`ci.yml:109-114`、`release-gate.js:27,30-48`） | `DATABASE_URL: file:./dev.db`（`release-gate.js:27`）。`check-standard-record-policy.js:18-32` が SQLite を実クエリ | 🟡 | 同一 run で作った使い捨て SQLite を検査している。**本番 PostgreSQL のデータ状態は一切見ていない。** 例えば本番の重複 `officialUrl` は検知されない | 分類は 🟡 で妥当。ゲート名から「本番データの検査」と誤読されないよう文書側で区別する |
...
| 24 | preview smoke（`ci.yml:138-153`） | `release-smoke.js:27` の `fetch` だが対象は同一 job で起動した `127.0.0.1:3100`（`ci.yml:144`） | 🟡 | 同一 run で起動した preview サーバを検査している。本番の挙動とは独立 | 分類どおり。#16（本番 smoke）と併存しているため実害は小さい |
...
副次的な実測として、同じ SARIF の6件は**実効 level が `warning`** であり、`level` を閾値にする実装なら security-severity 7.8 の指摘も含めて全件が素通りする。ここは実フィールドと実効値を分けて書く必要がある — 実測（run `31555165656` の artifact `9125778552`）では **6 件とも `result.level` 自体が欠落**しており、`warning` は `rule.defaultConfiguration.level` から来た値である。「`level` が `warning` だった」と書くと、値が入っていたのか欠落の既定値なのかが再現できない。**欠落と値を同じ記述へ潰すのは、本節が摘発している欠陥と同型である。**（同じ SARIF で rule 87 件中 2 件は `defaultConfiguration.level` も持たない。level を厳格化する実装は、欠落を構造エラーにせず既定へ落とす経路を残さなければ実データで落ちる。）さらに `repos/.../code-scanning/alerts` は **403「Code scanning is not enabled for this repository」**を返す。アラート一覧が空なのは、指摘が無いからではなく、保存先が設定で無効だからである。
...
walkthrough が壊れる実測。#148 の CodeRabbit コメントは**1件のみ**（`created=2026-08-11T23:46:50Z` / `updated=2026-08-12T00:19:33Z`）で、読めるのは最後の増分の申告だけである。過去の申告レンジは残らない。さらに #143 では walkthrough コメント本体（7,447 文字）は存在するのに `Recent review info` / `Commits` / `Files selected` の**どのブロックも出力されていない**。**申告の不在をカバー不足と読むと誤判定する** — 当時の #143 は実際に読まれていた（`reviews[].commit_id` = `0ab9bae` @23:22:02Z、これはその時点の head と一致）。

したがって判定はこの順で行う。

1. `reviews[].commit_id` に**現 head** があるか → あれば「その head は読まれた」（確定・非破壊）
2. 無ければ、次の**3つすべて**を満たすか → 満たせば「その増分は読まれた」
   1. 申告レンジの終端が現 head と一致する
   2. `Files selected (N)` が**そのレンジの差分ファイル数**と一致する
   3. コメント本文に**実体のある walkthrough がある**こと、すなわち `grep -c '^## Walkthrough'` が **1 以上**であること
3. どれも満たさなければ **「未カバー」ではなく「証明不能」**

**段2-3 は後から足した条件であり、かつ「存在」の形で書く。** 初版の段2は 1・2 の2条件しか課しておらず、それでは不十分だった。rate limit 通知の中にも `Files selected (N)` は現れるが、**それは待ち行列の宣言であってレビュー済み範囲の申告ではない**。CodeRabbit は「これから読む予定のファイル」を数えて表示し、実際には読まずに終わる。したがって 1・2 だけを見ると、**一度も読まれていない PR が「増分は読まれた」と判定される**。

段2-3 を「`rate limited by coderabbit.ai` を**含まない**こと」という**不在**の形で書いてはならない。通知の文言が変われば grep は当たらなくなり、条件は**通る側へ倒れる**。存在（`## Walkthrough` が 1 以上）で書けば、同じ変化で条件は**落ちる側へ倒れる**。これは §3.5 の不変条件（0件を合格にしない）と同じ向きの要求である。

反例は PR #151 で得た。申告レンジ終端 `0c07d67` は現 head と一致し、`Files selected (2)` は `compare/bebfd6f...0c07d67` の差分ファイル数 2 とも一致する。それでもコメント本体は rate limit 通知に包まれており、**実体のある `## Walkthrough` は 0 行**、formal review も 0 件である。読まれた証拠は何も無い。

これは本節が形態Bとして記述している欠陥そのもの — 表示された件数が、実際に評価された対象の件数を意味しない — を、**判定手順の側で踏んでいた**。判定式を書く行為も検査であり、検査は自分自身を免除されない。

> 📌 **訂正の記録。** この誤った段2と、それに基づく下記の「6/6 が証明可能」という結論は、CTO セッションが提示した分析に含まれていた誤りである。同セッションが PR #151 で自ら反証し、訂正を指示した。事実として記録する。
...
**実測（時点 2026-08-12T01:13Z、集合 = その時点の未マージ PR 全数 8 件）。** 判別子は `grep -c 'rate limited by coderabbit.ai'` と `grep -c '^## Walkthrough'` の突き合わせで測った:

| PR | 当時の head | rate limit 包み | 実 `## Walkthrough` | formal reviews | 判定 |
| --- | --- | --- | --- | --- | --- |
| #148 | `aaf0586` | あり | 1 | 2（`71eba41` / `aaf0586`） | ✅ 証明済（段1・head 一致） |
| #149 | `7c8ca25` | なし | 1 | 0 | ✅ 証明済（段2・3条件すべて充足） |
| #143 | `92cde47` | なし | 1 | 1（`0ab9bae`） | ❌ **証明不能**（review は `0ab9bae`、head は `92cde47`） |
| #130 | `9e3d70d` | なし | 0 | 0 | ❌ 証明不能 |
| #145 | `583a9eb` | あり | 0 | 0 | ❌ 証明不能 |
| #146 | `fec267b` | あり | 0 | 0 | ❌ 証明不能 |
| #150 | `1b8aae7` | あり | 0 | 0 | ❌ 証明不能 |
| #151 | `fc5eaa1` | あり | 0 | 0 | ❌ 証明不能 |

**この時点・この集合に対して、証明可能は 8 件中 2 件である。** 初版はここを「6/6 が証明可能」と書いていたが、これは訂正前の段2（rate limit 包みを見ない 2 条件版）で判定した結果であり、#145・#146・#150 の ✅ は**待ち行列の宣言を申告と読み違えたもの**だった。CTO セッションの再測では 3/8、本セッションの再測（上表）では 2/8 になる。差は #143 で、`0ab9bae` に付いた review の後に 2 回 push したため head が動いた。

**比率は時点と集合を伴わなければ意味を持たない。** 上の「2/8」は 01:13Z の未マージ 8 件に対する値であり、PR が増減すれば分母が変わる。分母を書かずに比率だけを引用すると、別の時点の別の集合へ持ち越されて誤用される。

**この差そのものが本節の主題である。** 「証明済み」は **commit に固定された性質**であって PR に固定された性質ではない。head が動けば証明は失効し、PR 番号で覚えていると失効に気付かない。判定は必ず現 head と突き合わせて測り直す。

**追記（時点 2026-08-12T01:43Z、対象 = PR #143 単体）。** 同じ #143 で形態Bがもう一度、別の面から出た。

- CodeRabbit の checks 行は `pass`（Review completed）だが、同時点で formal review は `0ab9bae` に固定されたままであり、head はその後 2 回の push とマージで動いている。**checks 行の緑は、現 head が読まれたことを意味しない。**
- この PR の review thread 5 件は、**すべて `coderabbitai[bot]` 自身が resolve していた**。実測は 01:42Z に unresolved=2、01:43Z に unresolved=0 で、`resolvedBy` は 5 件とも bot である。人手の裁定は 1 件も無い。
- したがって branch protection の `required_conversation_resolution=true` は、**「誰かが閉じた」しか要求しておらず、審議が行われたことを保証しない**。この必須条件の緑を「レビュー指摘は合意のうえ解決済み」と読むと、§3.5 と同じ形（検査が成立していないのに緑）になる。
- 実害の形も観測した。閉じられたスレッドの中には**内容が生きているもの**が含まれる。本 PR では「ゲートを非必須化するセキュリティ例外を、人間の承認付きで管理せよ」という指摘（§17 相当）が bot に閉じられ、スレッド一覧から消えた。

判定への反映: `unresolved == 0` を審議の成立として読まない。`resolvedBy` を引き、bot が閉じたものは**未審議**として扱う。閉じられて消えた指摘の生存確認は、スレッド一覧ではなく **PR 本文の残課題欄**で行う。

**区分は残す。廃止しない。** 初版は「証明不能 0 件だからこの判定式を阻止条件にしても誰も止まらない」と書いていたが、正しくは**上表の時点で 5 件が該当する**（#130 / #145 / #146 / #150 / #151。#143 を含めれば 6 件）。阻止条件に置けば実際に止まる。止まること自体は誤りではない — 止まるべきものが止まっているのであって、**「誰も止まらないから安全な判定式だ」という論法は、判定式が何も判定していない場合にも同じく成立する**。

なお、commits が 3 でも全レンジを申告し直す場合がある（#149）ため、「多段 push なら必ず証明不能」は成り立たない。増分申告（base ≠ merge-base）の形は #148 で実在する。
...
> 🧪 **仮説（未確定・反証条件つき）。** 申告 base を決めているのは「直前に formal review を打った commit」ではないか。既存 review が無い #149 は申告 base が merge-base、既存 review がある #148 だけが直前 review の commit（`71eba41` @23:52:35Z）を base にしている。**反証条件を予測できる**: review が X に付いた後、**指摘の無い push が2回以上**続けば、中間の Y は review にも申告にも残らない。#151 を含め、現時点で反証例は未観測である。
>
> ただし**支持データは当初思ったより薄い**。この仮説を最初に支えていた #145・#146・#150 の「申告 base = merge-base」という観測は、いずれも rate limit 通知の中の待ち行列宣言を申告として読んだものであり、証拠として使えない。実体のある walkthrough を伴う観測は #148 と #149 の**2点しかない**。
>
> **この仮説は証明可能性についての主張ではない。** 申告 base がどう決まるかが分かっても、それは「読まれた」ことの証拠を1つも増やさない。仮説から「証明不能は構造的に生じにくい」を導くことはできず、実測はむしろ逆（上表の時点で 8 件中 6 件が証明不能）である。機序の説明を、証拠の代わりに使ってはならない — 単一の指標から機序を断定するのは、本節が指摘している誤りそのものである。
...
# 是正後は neon-backup.yml:195 に --restore-drill-status がヒットする（= 供給経路が存在する）
