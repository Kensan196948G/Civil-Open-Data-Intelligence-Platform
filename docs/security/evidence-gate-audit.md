...
2. 無ければ、次の**3つすべて**を満たすか → 満たせば「その増分は読まれた」
   ...
   3. **そのコメントが `rate limited by coderabbit.ai` の通知に包まれていない**（＝実体のある `## Walkthrough` を伴う）
3. どれも満たさなければ **「未カバー」ではなく「証明不能」**

**段2-3 は後から足した条件である。** 初版の段2は 1・2 の2条件しか課しておらず、それでは不十分だった。rate limit 通知の中にも `Files selected (N)` は現れるが、**それは待ち行列の宣言であってレビュー済み範囲の申告ではない**。CodeRabbit は「これから読む予定のファイル」を数えて表示し、実際には読まずに終わる。したがって 1・2 だけを見ると、**一度も読まれていない PR が「増分は読まれた」と判定される**。
...
現況の実測（**2026-08-12T01:13Z**、未マージ PR 全数）。判別子は「コメントが rate limit 通知に包まれているか」と「実体のある `## Walkthrough` があるか」で、`grep -c 'rate limited by coderabbit.ai'` と `grep -c '^## Walkthrough'` を突き合わせて測った:

| PR | head | rate limit 包み | 実 `## Walkthrough` | formal reviews | 判定 |
| --- | --- | --- | --- | --- | --- |
...
**証明可能は 8 件中 2 件である。** 初版はここを「6/6 が証明可能」と書いていたが、これは訂正前の段2（rate limit 包みを見ない 2 条件版）で判定した結果であり、#145・#146・#150 の ✅ は**待ち行列の宣言を申告と読み違えたもの**だった。CTO セッションの再測では 3/8、本セッションの再測（上表）では 2/8 になる。差は #143 で、`0ab9bae` に付いた review の後に 2 回 push したため head が動いた。
...
**区分は残す。廃止しない。** 初版は「証明不能 0 件だからこの判定式を阻止条件にしても誰も止まらない」と書いていたが、正しくは**現状 5 件が該当する**（#130 / #145 / #146 / #150 / #151。#143 を含めれば 6 件）。阻止条件に置けば実際に止まる。止まること自体は誤りではない — 止まるべきものが止まっているのであって、**「誰も止まらないから安全な判定式だ」という論法は、判定式が何も判定していない場合にも同じく成立する**。
...
>
> **この仮説は証明可能性についての主張ではない。** 申告 base がどう決まるかが分かっても、それは「読まれた」ことの証拠を1つも増やさない。仮説から「証明不能は構造的に生じにくい」を導くことはできず、実測はむしろ逆（8件中6件が証明不能）である。機序の説明を、証拠の代わりに使ってはならない — 単一の指標から機序を断定するのは、本節が指摘している誤りそのものである。
