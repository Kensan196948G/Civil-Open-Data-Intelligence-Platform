...
> 🧪 **仮説（未確定・反証条件つき）。** 申告 base を決めているのは「直前に formal review を打った commit」ではないか。上表と整合する: 既存 review が無い #145・#146・#149・#150 はいずれも申告 base が merge-base、既存 review がある #148 だけが直前 review の commit（`71eba41` @23:52:35Z）を base にしている。これが正しければ、増分申告は**直前 review が存在するときにしか起きず**、その review は `reviews[].commit_id` に破壊されずに残るため、`base..X`（review が証明）＋ `X..head`（申告が証明）で鎖が閉じる — 「証明不能」は構造的に生じにくい。**反証条件も予測できる**: review が X に付いた後、**指摘の無い push が2回以上**続けば、中間の Y は review にも申告にも残らず証明不能になる。**現時点で未観測**。データ点は6件しかないので、これは仮説として置く。単一の指標から機序を断定するのは、本節が指摘している誤りそのものである。
