...
| MVP (レビュー公開) | 関係者レビュー | `https://codip-mvp.mirai-dx-platform.com`。**現行実体: Cloudflare Tunnel（`codip-mvp-cloudflared.service`）→ `localhost:18801`（`codip-mvp.service` = `next start`、SQLite `dev.db`）**。旧方式（Worker `codip-mvp` + Workers Custom Domains + Neon branch `mvp-20260813`）は `docs/runbooks/cloudflare-mvp.md` §0.1 に実測記録あり |
...
| ファイル | 内容 |
| --- | --- |
| `wrangler.jsonc` | Workers実行構成。`env.preview`/`env.production` named environment、production route `odip.mirai-dx-platform.com/*`、Hyperdrive binding宣言 (productionは実ID反映済み。Secrets値は含めない) |
...
| node-preview | GitHub Actions `node-preview` job。Dockerを使わず、SQLite preview DBをmigrate/seedし、`next start` + `release:smoke` で直接起動経路を検証。Issue #35のbranch protection差し替え候補 |
