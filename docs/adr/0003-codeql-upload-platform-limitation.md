...
2. その run が **Code Scanning 側で正常に処理されたこと**を確認する
   （run が完了し、解析結果の取り込みがエラーになっていないこと）。
   これはアラートの有無とは**独立した信号**であり、次の 3 を切り分けるために先に見る
3. その run で **その指摘が Code Scanning 画面に現れること**を目視する。
   件数と rule id を控える（画面が空でないことではなく、**期待した指摘であること**を見る）。
   出なかった場合、2 が成功していれば「取り込みは通ったが検出されなかった」、
   2 が失敗していれば「取り込みが壊れている」と切り分けられる
4. A を選んだ場合は artifact の生成と取得が両方成功することを確認する
5. `node scripts/tools/check-github-actions-contract.js` が通ることを確認する
6. canary ブランチを削除し、`main` に埋め込みコードが残っていないことを確認する

なお、`results` が空の有効な SARIF ではアラートは表示されない。これは正常な状態であり、
**それ自体を失敗と読んではならない**。本手順が既知の finding を植えることを必須にしているのは、
検出0件という観測を成功にも失敗にも解釈しなくて済む状態を作るためである。
「アラートが出ること」を条件にできるのは、**出るはずの指摘を自分で置いた場合に限る**。
