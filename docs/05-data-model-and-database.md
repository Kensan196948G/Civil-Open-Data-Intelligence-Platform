...
| テーブル | 内容 |
| --- | --- |
| `standard_records` | 標準化済み共通レコード |
| `standard_record_versions` | 差分・履歴 |
| `raw_objects` | 原本ファイル参照 |
| `licenses` | ライセンス・出典表記 |
| `source_schemas` | 公開元の項目定義 |
| `field_mappings` | 公開元項目と共通項目の対応 |
| `ingestion_jobs` | 取得ジョブ |
| `ingestion_job_logs` | ジョブログ |

PostgreSQL/PostGIS移行用の準備資材として、`prisma/postgresql/schema.prisma` と `prisma/postgresql/migrations/20260713113000_init/migration.sql` を追加する。現行SQLite実装を壊さず、次を検証できる。
...
| 標準レコード読取 | PostgreSQL/PostGIS環境では `/api/v1` が `standard_records` を読取可能 |
...
本番でPostgreSQLを使うには、Prisma Client生成元をPostgreSQL schemaへ切り替えるか、PostgreSQL schemaを正本へ昇格するリリース判断が必要である。
