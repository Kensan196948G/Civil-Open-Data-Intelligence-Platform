...
> 🔐 **302の読み分け（2026-09-08 追加 / Issue #207 のRCA）**: 302は常に「Access境界＝期待動作」ではない。**service token設定済みで302**なら、Accessがそのtokenを受け入れていない状態であり、監視credential/policy側の障害である。probeはこの場合「**Cloudflare Access service token rejected**」を出力する。初動は次の順で確認する。
>
...
> 2. service tokenが失効（`expires_on`）していないか。
