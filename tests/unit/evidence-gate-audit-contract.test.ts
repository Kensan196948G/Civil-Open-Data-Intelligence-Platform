...
describe("production evidence variables match the audited list", () => {
   ...
   * That proxy is wrong in a way backend's remediation exposed: 7f72626
   * (PR #137, Issue #128) keeps `evidenceState` as a deliberate presence check
   * and moves the rows onto a per-key format validator. No external call is
   * ever added, so the proxy pins the document to a claim that has become
   * false — and it stays green while doing so. Measure the row path instead.
   */
