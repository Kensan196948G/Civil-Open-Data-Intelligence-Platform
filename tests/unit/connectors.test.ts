...
describe("ホスト判定の契約", () => {
   ...
   *
   * この述語は意図的に広い。ホスト判定でない `array.includes(x)` でも赤くなる。
   * **そのとき例外リスト (許可ファイル名・許可行の列挙) を足してはならない。**
   * 足した瞬間、この検査が測るものは「文字列包含でホストを判定していないこと」から
   * 「リストに載っていること」へすり替わり、載せれば何でも通る経路になる。
   *
   * 正しい対処は通す条件を増やすことではなく、**述語の対象を狭める**こと。
   * 例: 全行を見るのをやめ、`new URL()` を通していない URL 由来の変数
   * (`url` / `value` / `source.*Url` 等) に対する `.includes(` だけを拾う。
   * 検出対象を絞る変更は「何を検査しているか」を保つが、例外リストは保たない。
   */
