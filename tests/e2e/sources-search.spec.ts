...
test.describe("データソース一覧・検索", () => {
  test("一覧に seed データが表示される", async ({ page }) => {
    ...
    await expect(page.getByRole("link", { name: "国土数値情報" })).toBeVisible();
    await expect(page.getByRole("link", { name: "PLATEAU" })).toBeVisible();
  });
  ...
  test("キーワード検索で絞り込める", async ({ page }) => {
    ...
    await expect(page.getByRole("link", { name: "PLATEAU" })).not.toBeVisible();
  });
  ...
  test("APIキー要否で絞り込める", async ({ page }) => {
    ...
    await expect(page.getByRole("link", { name: "国土数値情報" })).not.toBeVisible();
  });
});
