...
//
// ⚠️ 2026-09-09 訂正: 以前ここには「レイヤー切替・緯度経度入力パネル・実標高 API
// 呼び出しは廃止された」と書かれていたが、`src/components/MapView.tsx` には
// データレイヤーの表示切替・透明度スライダー・矩形検索・属性検索・時間フィルタが
// 現存し、UI にも表示される。この誤った記述が「廃止済みだからテスト不要」という
// 前提を作り、これらの機能はテストが 1 件も無い状態だった。
// 追加分は tests/e2e/map-layer-controls.spec.ts にある。
//
// この spec が受け持つのは地図そのものの描画と出典表記。
