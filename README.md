...
| 🗺️ 地図表示       | 地理院タイル表示、クリック地点の標高取得、GeoJSON表示ができます |
| 🔌 コネクタ方式     | データソース種別ごとの接続確認処理を拡張できます（gsi/jma/e-Stat等6種+汎用） |
| 🤖 AI支援        | データ概要の要約や活用候補の提案を補助します（将来拡張）       |

---

## 🌐 初期対象データ

seed データとして **56件** の公開データソース（提供元20機関）を収録しています。
以下のような公開データ・公開APIを登録対象とします。
...
E2Eテスト（初回は `npx playwright install chromium` が必要）：
...
| フェーズ    | 内容                               | 状態 |
| ------- | -------------------------------- | --- |
| Phase 1 | API台帳、検索、接続確認、取得ログ               | ✅ 完了 |
| Phase 2 | ダッシュボード、品質評価、タグ管理                | ✅ 完了 |
| Phase 3 | 地図表示、GeoJSON表示                   | ✅ 完了 |
| Phase 4 | AIによる要約・タグ推薦                     | 未着手 |
| Phase 5 | 工事候補地リスクチェックとの連携                 | 未着手 |
| Phase 6 | 気象・河川・施工判断支援との連携                 | 未着手 |
| Phase 7 | PostgreSQL / PostGIS対応           | 未着手 |
| Phase 8 | 社内CDE・SharePoint・DirectCloud連携検討 | 未着手 |
