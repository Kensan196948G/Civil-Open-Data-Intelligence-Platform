...
export async function GET(request: NextRequest) {
  const rate = checkRateLimit("api:v1:observations:weather", clientIdentifier(request), 120, 60_000);
  ...
  if (!row) {
    return NextResponse.json({ error: { code: "not_found", message: "気象観測データがありません" } }, { status: 404 });
  }
